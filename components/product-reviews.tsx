...
const products = {
  "イヤホン": ["QCY-T1C完全ワイヤレスイヤホン第3/4世代","Test"],
  "加湿器": ["小型加湿器S08"],
} as const;
...
export function ProductReviewsComponent() {
  ...
  const [expandedRightIndices, setRightExpandedIndices] = useState<Record<number, boolean>>({});

  const toggleExpand = (index: number) => {
    setExpandedIndices((prev) => ({
      ...prev,
      [index]: !prev[index], // 切换当前索引的状态
    }));
  };

  const toggleRightExpand = (index: number) => {
    ...
    return (
      <div>
        {sentences.map((sentence, index) => (
            <span key={index} className={selectedAspect && sentence.includes(selectedAspect) ? "bg-red-500":""}>
            {sentence}{' '}
          </span>
        ))}
      </div>
  ...
  const renderReviewWithBlueHighlight = (review:string) => {
    ...
    return (
      <div>
        {sentences.map((sentence, index) => (
            <span key={index} className={selectedAspect && sentence.includes(selectedAspect) ? 'bg-blue-500' : ''}>
            {sentence}{' '}
          </span>
        ))}
      </div>
...
{/*       
       ...
       <Modal isOpen={isOpen} onRequestClose={closeModal} className="modal">
        ...
        <div className="w-full flex justify-between ">
          ...
          {calculateKength(PReviews)}
           <div className="text-red-500 text-left ml-0">{selectedAspect}(Positive)
           </div>
          </div>
          <div className='flex items-center'>
          <ThumbsDown className="text-blue-500" />
          {calculateKength(NReviews)}
          <div className="text-blue-500 text-right mr-0">{selectedAspect}(Negative)</div>
          </div>
        </div>}
            ...
            {PositiveData.map((review, index) => (
              <div key={index} className="border-b py-2 last:border-b-0">
                <div className="flex flex-col">
                  <div>
                    <div>
                     {!expandedIndices[index] && showReviewWithclickaspect(review)}
                    </div>
            ...
            {NegativeData.map((review, index) => (
              <div key={index} className="border-b py-2 last:border-b-0">
                <div className="flex flex-col">
                  <div>
                   <div>
                     {!expandedRightIndices[index] && showReviewWithclickaspect(review)}
                    </div>
                  {expandedRightIndices[index] &&(
                  <div>
                   ...
                   <button
                       className="font-bold ml-2"
                      //  className="text-blue-500 ml-2"
                       onClick={()=>toggleRightExpand(index)}>
                          {expandedRightIndices[index] ? 'Show Less' : 'Show More'}
                    </button>
                  </div>
                  <div className="text-right mt-2 text-sm text-gray-500">
